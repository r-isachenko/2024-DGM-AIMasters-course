...
\begin{frame}{Stochastic differential equation (SDE)}
	...
	\vspace{-0.5cm}
	\begin{itemize}
		 \item $\mathbf{f}(\bx, t): \bbR^m \times \bbR \rightarrow \bbR^m$ is the \textbf{drift} function of $\bx(t)$.
...
\begin{frame}{Stochastic differential equation (SDE)}
	...
	\begin{itemize}
		...
		\item How to get \textbf{the distribution path} $p(\bx, t)$ for $\bx(t)$?
	\end{itemize}
\end{frame}
%=======
\begin{frame}{Stochastic differential equation (SDE)}
...
\begin{frame}{Reverse ODE}
	...
	\vspace{-0.5cm}
	\begin{itemize}
		\item Here $dt$ could be $>0$ or $<0$. It is straightforward to revert ODE.
...
\begin{frame}{Reverse ODE}
	\begin{block}{Proof}
		...
		Define the probability path for the reverse process $q(\bx, \tau) = p(\bx, 1 - \tau) = p(\bx, t)$ (here $\tau = 1 - t$).
		{\footnotesize
		\begin{multline*}
			\frac{\partial q(\bx, \tau)}{\partial \tau} = \frac{\partial p(\bx, 1 - \tau)}{\partial \tau} = - \frac{\partial p(\bx,  t)}{\partial t} = \\
			= - \text{tr}\left(- \frac{\partial}{\partial \bx} \bigl[ \mathbf{f}(\bx, t) p(\bx, t)\bigr] + \frac{1}{2} g^2(t) \frac{\partial^2 p(\bx, t)}{\partial \bx^2} \right) = \\
			= \text{tr}\left(\frac{\partial}{\partial \bx} \bigl[ \mathbf{f}(\bx, 1 - \tau) q(\bx, \tau)\bigr] - \frac{1}{2} g^2(1 - \tau) \frac{\partial^2 q(\bx, \tau)}{\partial \bx^2} \right) = \\
			= \text{tr}\left(\frac{\partial}{\partial \bx} \bigl[ \mathbf{f}(\bx, 1 - \tau) q(\bx, \tau)\bigr] - g^2(1 - \tau) \frac{\partial^2 q(\bx, \tau)}{\partial \bx^2} + \frac{1}{2} g^2(1 - \tau) \frac{\partial^2 q(\bx, \tau)}{\partial \bx^2} \right) = \\
			= \text{tr}\left(\frac{\partial}{\partial \bx} \left[ \left( {\color{teal}\mathbf{f}(\bx, 1 - \tau) - g^2(1 - \tau) \frac{\partial \log q(\bx, \tau)}{\partial \bx}}\right) q(\bx, \tau)\right] + \frac{1}{2} {\color{olive}g^2(1 - \tau)} \frac{\partial^2 q(\bx, \tau)}{\partial \bx^2} \right)
		\end{multline*}
		}
		\[
			d\bx = \left( {\color{teal}\mathbf{f}(\bx, 1 - \tau) - g^2(1 - \tau) \frac{\partial \log q(\bx, \tau)}{\partial \bx}} \right) d\tau + {\color{olive}g(1 - \tau)} d \bw
		\]
	\end{block}
	\myfootnotewithlink{https://arxiv.org/abs/2011.13456}{Song Y., et al. Score-Based Generative Modeling through Stochastic Differential Equations, 2020}
\end{frame}
...
\begin{frame}{Reverse ODE}
	...
	\begin{block}{Proof (continued)}
		\vspace{-0.7cm}
		\[
			d\bx = \left(\mathbf{f}(\bx, 1 - \tau) - g^2(1 - \tau) \frac{\partial \log q(\bx, \tau)}{\partial \bx}\right) d\tau + g(1 - \tau) d \bw
		\]
		\[
